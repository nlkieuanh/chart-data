...
function advApplyCompanySelection(item) {
  ...
  if (api && typeof api.setCompany === "function") {
...
function advApplyMetricSelection(item) {
  ...
  if (api && typeof api.setMetric === "function") {
    api.setMetric(value);
  }
}
...
function advInitChart(wrapper, jsonUrl) {
  ...
  // Attach chart API to the nearest card-block-wrap so dropdowns in the same section use this instance
  var section = wrapper.closest(".card-block-wrap");
  if (section) {
    section._advChartApi = chartApi;
  }
