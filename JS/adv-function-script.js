...
function advInitChart(wrapper, jsonUrl) {
  ...
  advLoadNewJSON(jsonUrl)
    .then(function (json) {
      ...
      // Save bounds for external date UI (per card)
      card._advDateBounds = {
        min: allDates[0],
...
/* ============================================================
   10. Scoped dropdowns (company / metric) per card-block-wrap
   ============================================================ */

function advInitCompanyDropdown(cardEl, companies) {
  if (!cardEl || !Array.isArray(companies) || !companies.length) return;

  var wrappers = cardEl.querySelectorAll(".company-dd-link-select");
...
function advInitMetricDropdown(cardEl, metrics, labelsMap) {
  if (!cardEl || !Array.isArray(metrics) || !metrics.length) return;

  var wrappers = cardEl.querySelectorAll(".chart-metric-dd-select");
  wrappers.forEach(function (wrapper) {
    var scriptHolder = wrapper.querySelector(".chart-metric-select-script");
    var listContainer = scriptHolder ? scriptHolder.parentElement : wrapper;
...
/* Override global click handler for company + metric dropdowns
   (will coexist with previous listener but use the new scoped apply*) */
document.addEventListener("click", function (event) {
  var companyItem = event.target.closest(".company-dd-link-select [data-dropdown]");
  if (companyItem) {
    advApplyCompanySelection(companyItem);
...
/* ============================================================
   11. Date-range dropdown (flatpickr) scoped per card
   ============================================================ */
