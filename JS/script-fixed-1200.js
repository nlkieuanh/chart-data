...
function advBuildChartPayload(options) {
  ...
  var yourCompany =
    seriesList.find(function (s) { return s.name === "Your Company"; }) ||
    seriesList[0] ||
    null;

  var competitors = yourCompany
    ? seriesList.filter(function (s) { return s !== yourCompany; })
    : seriesList;
...
document.addEventListener("click", function (event) {
  ...
  if (companyItem) {
    ...
    if (dd1 && window.$) $(dd1).triggerHandler("w-close.w-dropdown");
    return;
  }
  ...
  if (metricItem) {
    ...
    if (dd2 && window.$) $(dd2).triggerHandler("w-close.w-dropdown");
  }
});

/* ============================================================
   9. Init Chart + Controller (PATCHED)
   ============================================================ */
...
function advInitChart(wrapper, jsonUrl) {
  ...
  advLoadNewJSON(jsonUrl)
    .then(function (json) {
      ...
      let metricList = [];
      if (Array.isArray(jsonData.baseMetrics)) {
  ...
  function connectModeSwitch() {
    ...
    function setActive(activeEl, others) {
      ...
      (others || []).forEach((el) => el.classList.remove("is-active"));
    }
    ...
    tbody.addEventListener("change", function (event) {
      ...
      if (cb.checked) {
        ...
        if (stillChecked.length) {
          ...
          if (first) {
            first.checked = true;
            selectedChannels = [first.getAttribute("data-adv-channel")];
          }
        }
      }
      ...
      /* 
        PATCH #3:
        - Remove: renderChartAndTable();
        - Add: only update chart
      */
  ...
  /* ---------------- FULL RENDER: chart + table (patched) ---------------- */
  ...
  function renderChartAndTable() {
    ...
    if (tbody) {
      ...
      /* 
        PATCH #2:
        Replace advRenderChannelTable(...) with:
      */
      if (typeof card._advRebuildTable === "function") {
        card._advRebuildTable(currentCompanyId, dateIndexes, selectedChannels);
      }
