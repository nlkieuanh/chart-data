...
function advCreateCompanyBlock(container, company, valueType) {
  ...
  // Dynamic height theo số angles
  const anglesCount = Object.keys(company.anglesOffers).length;
  const calcHeight = Math.max(anglesCount * 30, 120); // bar 20px + spacing 10px
  ...
  if(mode==="direct"){
    ...
    data.competitors.forEach(c=>{
      datasets.push({
        ...
        borderDash:[4,2], fill:false, tension:0.3
      });
    });
  } else {
    datasets.push({
...
function advCreateStackedBarChart(ctx, data, mode, valueType){
  ...
  window[ctx.canvas.id+"Chart"]=new Chart(ctx,{
    ...
    options:{
      ...
      indexAxis:"y",
      scales:{
        x:{stacked:true,beginAtZero:true,max:valueType==="percent"?100:undefined,
           ticks:{callback:val=>valueType==="percent"?val+"%":val}},
        y:{stacked:true}
      }
    }
...
function advCreateHorizontalBarChart(ctx, data, mode, valueType){
  ...
  window[ctx.canvas.id+"Chart"]=new Chart(ctx,{
    ...
    options:{
      ...
      indexAxis:"y",
      scales:{
        x:{beginAtZero:true,max:valueType==="percent"?100:undefined,
           ticks:{callback:val=>valueType==="percent"?val+"%":val}}
      }
    }
  });
