 ...
 * - Shared date filter (per card-block-wrap)
 * - Shared channel selection (checkbox in table, per tab)
 * - Metric dropdown to choose which metric to plot in chart (per tab)
 ***********************************************************/
...
function advFilterDateRange(dates, startDate, endDate) {
  ...
  if (!start || !end) {
    return dates.map(function (_, i) { return i; });
  }
...
/* ============================================================
   7. Legacy table renderer (fallback only)
   ============================================================ */
...
function advApplyCompanySelection(item) {
  ...
  var tabSwitch = document.querySelector('[data-w-tab="' + value + '"]');
  if (tabSwitch) tabSwitch.click();

  var tabRoot =
    wrapper.closest(".w-tab-pane") ||
    wrapper.closest(".competitors-tab") ||
    wrapper.closest(".best-in-class-tab") ||
    wrapper.closest(".tab-content-flex") ||
    wrapper.closest(".card-block-wrap");

  if (!tabRoot) return;

  var chartWrapper = tabRoot.querySelector(".chart-canvas");
  var ctrl = chartWrapper && chartWrapper._advController;
...
function advInitMetricDropdown(scopeEl, metrics, labelsMap) {
  ...
  scopeEl.querySelectorAll(".chart-metric-dd-select").forEach(function (wrapper) {
...
/* ============================================================
   9. Init chart (per tab wrapper scope)
   ============================================================ */
...
function advInitChart(wrapper, jsonUrl) {
  ...
  if (card._advTabControllers.indexOf(wrapper) === -1) {
  ...
  advLoadNewJSON(jsonUrl)
    .then(function (json) {
      ...
      if (firstChannel && Array.isArray(firstChannel.companies) && firstChannel.companies.length) {
        ...
        advInitCompanyDropdown(wrapper.closest(".w-tab-pane") || wrapper.closest(".tab-content-flex") || card, companyList);
        if (!currentCompanyId) {
      ...
      if (metricList.length) {
        advInitMetricDropdown(wrapper.closest(".w-tab-pane") || wrapper.closest(".tab-content-flex") || card, metricList, metricLabels);
  ...
  const controller = {
    setDateRange: function (start, end) {
      ...
      if (tableRender && typeof tableRender._advRebuildTable === "function") {
        tableRender._advRebuildTable(currentCompanyId, dateIndexes, selectedChannels);
      }
    },
    ...
    setCompany: function (companyId) {
      ...
      if (tableRender && typeof tableRender._advRebuildTable === "function") {
        tableRender._advRebuildTable(companyId, dateIndexes, selectedChannels);
      }
    },
  ...
  function connectTableCheckbox() {
    ...
    tableRender.addEventListener("change", function (event) {
      ...
      var boxes = Array.prototype.slice.call(
        tbodyEl.querySelectorAll(".adv-channel-checkbox")
      );
  ...
  function renderChartAndTable(shouldRebuildTable) {
    if (!jsonData) return;
    ...
    if (shouldRebuildTable && tableRender && typeof tableRender._advRebuildTable === "function") {
...
(function () {
  ...
  function getControllers(wrapper) {
    var card = wrapper.closest(".card-block-wrap");
    if (!card) return [];
    var list = card._advTabControllers || [];
    return list.map(function (chartWrapper) {
      return chartWrapper && chartWrapper._advController;
    }).filter(Boolean);
  }
  ...
  function applyDateRangeSelection(item) {
    ...
    var ctrls = getControllers(wrapper);
    ...
    if (value === "default") {
      ...
      ctrls.forEach(function (c) {
        c.setDateRange(advToISODate(start), advToISODate(end));
      });
    }
    ...
    if (value === "lastMonth") {
      ...
      ctrls.forEach(function (c) {
        c.setDateRange(advToISODate(start2), advToISODate(end2));
      });
    }
  }
...
/* ============================================================
   11. Dynamic channel table per tab (advInitTable)
   ============================================================ */
      ...
      .then(function (json) {
        ...
        function buildRows(companyId, customIndexes, selectedChannels) {
          ...
          channels.forEach(function (channel, index) {
            ...
            var isChecked = currentSelectedChannels.indexOf(channel.id) !== -1;
            if (currentSelectedChannels.length === 0 && index === 0) {
              isChecked = true;
            }

            if (isChecked && newlyCheckedIds.indexOf(channel.id) === -1) {
          ...
          var ctrl = chartWrapper && chartWrapper._advController;
          if (ctrl && typeof ctrl.setChannels === "function" && newlyCheckedIds.length) {
            if (
              newlyCheckedIds.length !== currentSelectedChannels.length ||
              newlyCheckedIds.some(function (id) {
                return currentSelectedChannels.indexOf(id) === -1;
              })
            ) {
              ctrl.setChannels(newlyCheckedIds);
            }
          }
        }
        ...
        var initialChannelId = channels[0] ? [channels[0].id] : [];
